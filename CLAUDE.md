...
.venv/bin/pytest                                              # all tests (~3 s)
.venv/bin/pytest tests/test_siemens.py::test_main_orientation_and_ties   # single test
.venv/bin/pytest tests/test_viewer.py                         # GUI tests only (pytest-qt)
...
Dependency direction: `viewer` → `siemens` → `geometry` ← `volume`. Only `viewer.py` and `app.py` import Qt; everything else is pure numpy/scipy/nibabel.

GUI tests (`tests/test_viewer.py`) drive a real `MainWindow` with pytest-qt. `tests/conftest.py` forces `QT_QPA_PLATFORM=offscreen` (overriding the desktop's value, which would open real windows that a tiling WM resizes). The `window` fixture isolates `QSettings` to `tmp_path` and replaces `QMessageBox` with a recorder (`dialogs` fixture). Gotchas: `qtbot.waitActive`/`waitExposed` are context managers and do nothing if just called; two presses within `doubleClickInterval()` become a pyqtgraph double-click rather than a second drag.
...
- **State flow.** `MainWindow.pose` is the single source of truth for the voxel (`slice_point`/`follow_voxel` only control the views; change them via `set_slice_point`/`set_follow_voxel`). All edits (mouse, `ParameterPanel` fields, file loads) go through `MainWindow.set_pose`, which refreshes all views and the panel. A user edit must call `MainWindow.remember_pose()` *before* `set_pose`, or it can't be undone (drags call it once at drag start, so one drag is one undo step); programmatic updates such as `undo`/`redo` and `set_volume` don't. `VoxelPose` and `Plane` are frozen dataclasses: use `translated`/`rotated`/`with_size`/`through` to derive new ones instead of mutating.
- **File actions** (`open_t1`, `load_voxel`, `save_voxel`, `export_mask`, `export_figure`; the figure is `render_figure(scale)`, the views re-rendered at a higher device pixel ratio) wrap their work in `with self._reporting(action):`, which turns exceptions into an error dialog and any `warnings.warn` (e.g. the RDA RowVector/ColumnVector check) into a warning dialog, every time. Use `_choose_file`/`_remember_dir` for dialogs so the last folder is remembered.

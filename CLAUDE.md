...
- **Siemens boundary is siemens.py.** Siemens values live in LPS (`sPosition`, `sNormal`, `dInPlaneRot`); `LPS_RAS` (diag(-1,-1,1), its own inverse) converts. `phase_readout` is a port of the scanner's CalcPRS routine (following spec2nii) and derives phase/readout from the normal + in-plane rotation; `main_orientation` tie-breaking (Transverse > Coronal > Sagittal) affects it. `siemens_from_pose` inverts `pose_from_siemens` by recovering in-plane rotation from the phase axis. Orientation strings like `T > C -23.9 > S 4.5` are parsed/produced here too. Only transverse-based strings have been checked against real scanner data.
- **Views (viewer.py).** `VIEWS` defines three fixed anatomical `Plane`s (h = screen right, v = screen up, n = h×v) in radiological convention. Each `SliceView` re-centres its plane through the voxel centre on every `refresh`, draws `section_polygon(pose, plane)` (up to 6 vertices when the voxel is oblique; every vertex is a rotate handle), and caches the resliced image by plane offset. Mouse drags in `VoxelViewBox` become `request_move` (translate in-plane) or `request_rotate` (rotate about the view's normal `plane.n`).
- **State flow.** `MainWindow.pose` is the single source of truth. All edits (mouse, `ParameterPanel` fields, file loads) go through `MainWindow.set_pose`, which refreshes all views and the panel. `VoxelPose` and `Plane` are frozen dataclasses: use `translated`/`rotated`/`with_size`/`through` to derive new ones instead of mutating.
